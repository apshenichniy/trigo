...
@Test func nativeSampleBufferConversionPreservesHostClockOffsetAndMonoLevel() throws {
  ...
  let sample = try controlledAudioBuffer(
    sampleRate: 48_000,
    frames: 4_800,
    time: CMTime(seconds: 100.25, preferredTimescale: 1_000_000_000),
    value: 0.25
  )
...
func hardwareMicrophonePacketsDoNotCreatePeriodicHolesInPersistedPCM(
  ...
  let interiorGaps = (try captureIntervals(writer, role: .microphone))
    .filter {
...
@Test func conversionRetainsRealHostClockGapsAndResetsFilterHistory() throws {
  ...
  _ = try decoder.decode(
    controlledAudioBuffer(sampleRate: 48_000, frames: 512, time: origin, value: 0.9),
    origin: origin
  )
  let resumed = try decoder.decode(
    controlledAudioBuffer(
      sampleRate: 48_000,
      frames: 512,
      time: CMTime(seconds: 100.1, preferredTimescale: 1_000_000_000),
      value: 0
    ),
    origin: origin
  )
  ...
  for packet in 0..<1_000 {
    ...
    let decoded = try decoder.decode(
      controlledAudioBuffer(
        sampleRate: 48_000,
        frames: 512,
        time: CMTime(seconds: 100 + relative, preferredTimescale: 1_000_000_000),
        value: 0.25
      ),
      origin: origin
    )
    worstClockError = max(
      worstClockError,
      abs(decoded.startFrame - Int((relative * 16_000).rounded()))
    )
  }
