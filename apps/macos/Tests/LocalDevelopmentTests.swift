...
private func localConfiguration(_ url: URL) throws -> LocalDevelopmentConfiguration {
  try .init(
    worktreeId: localWorktree, namespaceId: localNamespace, serverURL: url, ownerToken: localToken)
}
private func temporarySupport() throws -> URL {
  let root = FileManager.default.temporaryDirectory.appending(
    path: "trigo-local-client-\(UUID())", directoryHint: .isDirectory)
...
struct LocalDevelopmentTests {
    ...
    for fixture in corpus where fixture.kind == "LocalDevelopmentBridge" {
      ...
      do {
        let configuration = try LocalDevelopmentConfiguration.load(
          url: file, worktree: localWorktree, variant: .dev)
  ...
  @Test func localNamespaceCannotReplaceOrdinaryCloudOrPersonalState() throws {
    ...
    let offline = try AppNamespace(
      variant: .dev, worktree: localWorktree, support: support, localDevelopment: local)
    ...
    #expect(throws: (any Error).self) {
      try AppNamespace(
        variant: .personal, worktree: localWorktree, support: support, localDevelopment: local)
    }
    #expect(throws: (any Error).self) {
      try AppNamespace(
        variant: .dev, worktree: "abcdef012345", support: support, localDevelopment: local)
    }
  }
  ...
  @Test func bridgeRejectsUnsafeFilesAndMismatchedWorktrees() throws {
    ...
    let bridge = LocalDevelopmentBridge(
      formatVersion: 1, worktreeId: localWorktree, namespaceId: localNamespace,
      serverURL: "http://127.0.0.1:19371", ownerToken: localToken)
    ...
    async throws
  {
    for (status, body, expected) in [
      ...
      let client = HTTPSStatusClient(
        timeout: 2, transportPolicy: .localDevelopment(try localConfiguration(url)))
    ...
    async throws
  {
    ...
    let namespace = try AppNamespace(
      variant: .dev, worktree: localWorktree, support: support, localDevelopment: configuration)
  ...
  @Test func realTransportRejectsRedirectsAndForbiddenOriginsBeforeSendingCredentials() async throws
  {
    let fixture = try await LoopbackResponse.start(
      status: 302, body: "", location: "http://127.0.0.1:19372/v1/status")
    defer { fixture.stop() }
    let url = try #require(fixture.url)
    let client = HTTPSStatusClient(
      timeout: 2, transportPolicy: .localDevelopment(try localConfiguration(url)))
...
struct LocalServerAcceptanceTests {
  ...
  func pairsRestoresAndRejectsWrongCredentialsAgainstTheActualLocalComposition() async throws {
    ...
    let config = try LocalDevelopmentConfiguration.load(
      url: URL(fileURLWithPath: path), worktree: worktree, variant: .dev)
    let support = try temporarySupport()
    defer { try? FileManager.default.removeItem(at: support) }
    let namespace = try AppNamespace(
      variant: .dev, worktree: worktree, support: support, localDevelopment: config)
    ...
    let unauthorized = await first.connect(
      serverURL: config.serverURL.absoluteString, token: "invalid")
    #expect(unauthorized.binding == nil)
    #expect(unauthorized.lastAttemptIssue == .unauthorized)
    let paired = await first.connect(
      serverURL: config.serverURL.absoluteString, token: config.ownerToken)
...
private final class LoopbackResponse: @unchecked Sendable {
  ...
  static func start(status: Int, body: String, location: String? = nil) async throws
    -> LoopbackResponse
  {
