...
func productionFinalMasterKillChild() async throws {
  ...
  for part in 0..<30 {
    ...
    for role: MediaSourceRole in [.microphone, .application] {
      try engine.receive(
        controlledAudioBuffer(sampleRate: 16_000, frames: 1600, time: time, value: 0.25),
        role: role
      )
    }
  ...
  let work = OperationIntent(
    operationID: finalizationOperation,
    archiveID: finalizationArchive,
    callID: session.callID,
    kind: .upload,
    payload: Data("complete master required".utf8)
  )
...
func productionFinalIndexSIGKILLRetainsMasterStopCauseAndPreparedWork(
  point: String,
  reason: String
)
  async throws
...
func productionStreamingPublicationRecoversExactBytesAndWork(
...
@Test func rawAndStreamingFinalizationShareRetainedWorkReasonAndSourceEvidence() async throws {
  ...
  await #expect(throws: LocalPersistenceError.self) {
    try await repository.finalizeCapture(
      callSnapshot: Contract.encode(wrongReason),
      audioManifest: audio,
      verifiedMaster: master
    )
  }
  ...
  await #expect(throws: LocalPersistenceError.invalidMediaProgress) {
    try await repository.finalizeCapture(
      callSnapshot: Contract.encode(wrongSource),
      audioManifest: audio,
      verifiedMaster: master
    )
  }
  ...
  await #expect(throws: immutableSnapshot) {
    try await repository.finalizeCapture(
      callSnapshot: Contract.encode(call),
      audioManifest: audio,
      verifiedMaster: master
    )
  }
  ...
  await #expect(throws: immutableSnapshot) {
    try await repository.finalizeCapture(
      callSnapshot: Contract.encode(changedMetadata),
      audioManifest: audio,
      verifiedMaster: master
    )
  }
