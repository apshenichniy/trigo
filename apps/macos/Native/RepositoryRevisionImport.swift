...
extension LocalRepository {
  public func prepareRevisionImport(_ bytes: Data, associatedWork: OperationIntent) async throws
    -> PreparedRevisionImport
  {
    ...
    if let retained = call.revisions.first(where: { $0.revisionId == revision.value.revisionId }) {
      ...
      call.revisions.append(
        .init(
          revisionId: revision.value.revisionId, createdAt: revision.value.createdAt,
    ...
    return .init(
      root: root, archiveID: archiveID, priorHash: priorHash, revision: revision,
  ...
  private func validateImportReferences(_ revision: TranscriptRevision, against call: CallDocument)
    throws
  {
    ...
    for retained in call.revisions {
      let hash = try requiredEvidence(
        identity: retained.revisionId, kind: "revision", callID: call.callId)
  ...
  @discardableResult
  public func commitRevisionImport(_ prepared: PreparedRevisionImport) async throws
    -> PublicationResult
  {
      ...
      try database.transaction(interruption: interruption) {
        if try semanticWorkExists(semanticID) {
          ...
          _ = try commitEvidence(
            identity: revision.value.revisionId, kind: "revision", callID: revision.value.callId,
        ...
        if retained.isEmpty {
          _ = try commitCall(
            prepared.snapshot.value, hash: prepared.snapshot.sha256, expected: prepared.priorHash)
        } else {
  ...
  @discardableResult
  public func importRevision(_ bytes: Data, associatedWork: OperationIntent) async throws
    -> PublicationResult
  {
  ...
  func validateSemanticWork(_ identity: String, operation: PreparedOperation?) throws {
    ...
    guard
      let row = try database.rows(
        "SELECT operation_id FROM semantic_work WHERE identity=?", [.text(identity)]
      ).first,
