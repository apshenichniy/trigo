...
public final class CaptureMediaWriter {
  ...
  public var durationMs: Int { (checkpoint.objects.last?.endMs ?? 0) + activeBytes / 64 }
  ...
  public func append(
    ...
    guard !interleaved.isEmpty, interleaved.count % 32 == 0 else { throw CaptureError.invalidAudio }
    guard interleaved.count / 32 <= profile.maxCallDurationMs - durationMs else {
    ...
    do {
      ...
      while offset < interleaved.count {
        ...
        let count = min(
          32_000, interleaved.count - offset,
          (profile.maxObjectBytes - 44 - activeBytes) / 2)
        ...
        try save()
        if activeBytes == profile.maxObjectBytes - 44 { try seal() }
        offset += count
      }
    } catch {
      closed = true
  ...
  private func seal() throws {
    ...
    let wave = waveHeader(pcmBytes: pcm.count) + pcm
  ...
  public static func recover(directory: URL) throws -> CaptureRecovery {
    ...
    guard stored.version == 1, isCanonicalIdentifier(stored.activeID),
      stored.objects.count <= 180, stored.fragments.count <= 60_000
    ...
    for object in stored.objects {
      guard isCanonicalIdentifier(object.objectID), object.index == objects.count,
        ...
        object.endMs - object.startMs <= 60_000, object.endMs <= 10_800_000,
        object.byteLength == 44 + (object.endMs - object.startMs) * 64,
        ...
        bytes.prefix(44) == waveHeader(pcmBytes: bytes.count - 44)
      else {
        rejected = true
        break
      }
    ...
    if !rejected && !stored.fragments.isEmpty {
      ...
      for fragment in stored.fragments {
        guard fragment.byteLength > 0, fragment.byteLength <= 64_000, fragment.byteLength % 64 == 0,
          pcm.count + fragment.byteLength <= 3_840_000,
      ...
      if !pcm.isEmpty {
        ...
        guard start + pcm.count / 64 <= 10_800_000 else { throw CaptureError.corruptCheckpoint }
        let object = CapturedMediaObject(
          objectID: id, index: objects.count, byteLength: wave.count,
          sha256: captureHash(wave), startMs: start, endMs: start + pcm.count / 64)
...
func waveHeader(pcmBytes: Int) -> Data {
  ...
  header.appendLittleEndian(UInt32(64_000))
