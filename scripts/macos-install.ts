...
export function nativeSigning(action: string, team: string | undefined, adHoc: boolean): string[] {
  if (adHoc) {
    if (team) throw new Error("Unset TRIGO_SIGNING_TEAM when explicitly using --ad-hoc");
...
export function installationDestination(
  ...
  if (variant !== "dev" && variant !== "personal") throw new Error("Invalid app variant");
  if (!/^[a-f0-9]{12}$/.test(worktree)) throw new Error("Invalid worktree identity");
  if (local && variant !== "dev") throw new Error("Local installations require the dev variant");
