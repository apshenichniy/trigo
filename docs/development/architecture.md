...
| ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| App lifetime            | `RecordingApplication` admits one process through `AppInstanceLease` before connection recovery, shortcut, readiness or capture setup. Namespace ownership lasts until process exit.                                                                  |
...
The native sources live under [`apps/macos/Native`](../../apps/macos/Native/).
...
| #56 final-source checks                   | [Current command/documentation acceptance](acceptance-56.md), full required gates and isolated 1h/3h production resource proofs including full extraction. Concurrent suite RSS is not the per-capture resource measurement.                                                                            |
| #57 installed acceptance                  | Signed final-source app, controlled non-private capture/recovery, unchanged relaunch and supported signed rebuild. The original Allow/Always Allow requester/cause remains unconfirmed. Use one stable worktree, bridge, identity and installed path; no global TCC/Keychain reset.                     |
...
| #10 product workflow                      | Production upload during capture, post-call ASR/sync, verified-receipt cleanup, transcript UI, compact measured recording feedback, double-Left-Option/Input Monitoring and real-call acceptance remain required.                                                                                       |
